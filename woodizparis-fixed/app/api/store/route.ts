    ...
    return NextResponse.json({
      ...
      data: {
        ...
        adminCredentials:  credRow
          ? { username: credRow.username, password: "" }
          : null,
      },
    });
  } catch (err) {
...
async function saveAdminCredentials(creds: any) {
  if (!creds) return;
  // Ne jamais écraser le hash par une chaîne vide ou le mot de passe en clair sans préfixe
  if (!creds.password) return;

  import("crypto").then(async ({ createHash }) => {
    const hash = createHash("sha256")
      .update(creds.password + (process.env.AUTH_SALT ?? "woodiz"))
      .digest("hex");
    await execute(
  ...
  }).catch((err) => console.error("[store] saveAdminCredentials failed:", err));
}
