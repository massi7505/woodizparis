  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (data) {
        ...
        // Ne pas restaurer les credentials depuis le serveur (password vide)
        if (data.adminCredentials?.username)
          useWoodizStore.setState((s) => ({
            adminCredentials: { ...s.adminCredentials, username: data.adminCredentials.username }
          }));
        isApplyingServerData = false;
      }
      ...
      // Stocker l'unsubscribe pour pouvoir nettoyer proprement
      unsubscribe = useWoodizStore.subscribe((state) => {
    ...
    // Cleanup: annuler le timeout et désabonner le store
    return () => {
      if (saveTimeout) clearTimeout(saveTimeout);
