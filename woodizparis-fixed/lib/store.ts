...
export interface Promo { id: number; title: string; price: string; badge: string; bg: string; textColor: string; priceColor?: string; active: boolean; image?: string; order: number; translations?: Record<string, { title: string; badge?: string; ctaText?: string }>; type?: "card" | "banner"; link?: string; bgImage?: string; ctaText?: string; }
...
export const useWoodizStore = create<WoodizStore>()(
  persist(
    (set, get) => ({
      ...
      // Le mot de passe réel est stocké dans MySQL. Cette valeur par défaut
      // n'est utilisée que si la DB est inaccessible (jamais exposé côté client).
    ...
    }),
    {
      ...
      migrate: (persistedState: any, version: number) => {
        ...
        if (version < 3) { if (!state.adminCredentials) state.adminCredentials = { username: "admin", password: "" }; if (!state.faqs) state.faqs = DEFAULT_FAQS; }
        if (version < 6) {
