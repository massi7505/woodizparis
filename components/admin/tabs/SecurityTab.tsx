...
import { Eye, EyeOff, Shield, Check, AlertTriangle } from "lucide-react";
...
export default function SecurityTab() {
  ...
  function validate() {
    ...
    if (form.currentPassword !== adminCredentials.password) errs.currentPassword = "Mot de passe actuel incorrect";
  ...
  function handleSave() {
    ...
    updateAdminCredentials({
      ...
      password: form.newPassword || adminCredentials.password,
    });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h2 className="font-bold text-slate-800" style={{ fontFamily: "Poppins, sans-serif" }}>Modifier les identifiants</h2>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">Actuel : <strong>{adminCredentials.username}</strong></p>
            </div>
            ...
            <div>
              ...
              {errors.currentPassword && (
                <p className="text-xs text-red-500 mt-1 flex items-center gap-1"><AlertTriangle size={11} /> {errors.currentPassword}</p>
              )}
            </div>

            {/* New password */}
            <div>
              <label className="form-label">Nouveau mot de passe <span className="text-slate-400 font-normal">(laisser vide pour ne pas changer)</span></label>
              <div className="relative">
                <input
            ...
            {form.newPassword && (
              <div>
                ...
                {form.confirmPassword && form.newPassword === form.confirmPassword && (
                  <p className="text-xs text-emerald-500 mt-1 flex items-center gap-1"><Check size={11} /> Les mots de passe correspondent</p>
                )}
            ...
            <button
              onClick={handleSave}
              className="w-full py-3 rounded-xl text-white text-sm font-semibold hover:opacity-90 transition-all flex items-center justify-center gap-2 mt-1"
              style={{ background: settings.primaryColor }}
            >
              <Check size={15} /> Sauvegarder les modifications
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
            <h2 className="font-bold text-slate-800 mb-4" style={{ fontFamily: "Poppins, sans-serif" }}>Session actuelle</h2>
            <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50">
                ...
                <span className="font-bold text-sm text-slate-800">{"•".repeat(Math.min(adminCredentials.password.length, 12))}</span>
              </div>
